...
// ─── Simulator for pre-launch demos ───────────────────────────────────────────
// Runs on a cron. Picks up any activity on active jobs and has the fake
// "sim*@thrive.local" candidates respond on a realistic delay:
...
//
// Idempotent: every branch checks state before acting, so running the cron a
...
// Fixed set of simulated candidate user_ids. Kept in sync with the DB seed
// (all follow 11111111-1111-4000-a000-00000000000N). Using an explicit list
// because PostgREST doesn't support LIKE against uuid columns without a cast.
const SIM_USER_IDS = [
...
// Pre-baked "signature": a tiny PNG encoding of a hand-drawn-looking loop so
// the signature certificate page has something to embed for sim candidates.
// Generated once, stored under signatures/_sim/default.png on first run.
...
export async function POST(req: NextRequest) {
  ...
  const counts = { applications: 0, interviewsConfirmed: 0, offersAccepted: 0, offersDeclined: 0, errors: 0 }
  ...
  try {
    ...
    for (const job of allJobs) {
      ...
      // Leave each job with 1–3 sim applications total; skip if already saturated.
      ...
      for (let i = 0; i < target - currentSimApps && available.length > 0; i++) {
        ...
        const screeningAnswers = Array.isArray(job.screening_questions)
          ? job.screening_questions.map((q: any) => {
              ...
              // Heuristic: yes/no questions usually open with these verbs.
  ...
  try {
    ...
    for (const app of apps || []) {
      // Only react if there's an actual interview on file (i.e. employer
      // invited the candidate), not just any application.
  ...
  try {
    ...
    for (const offer of offers || []) {
      ...
      if (roll < 0.65) {
        // Accept — swap offer_letter_url to a signed copy and flip status
      ...
      // else: leave pending to reflect real-world indecision
    }
  } catch (err) {
...
// Allow GET for manual pokes during local dev, gated on the same secret
